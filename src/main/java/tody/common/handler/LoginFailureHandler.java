...
public class LoginFailureHandler implements AuthenticationFailureHandler {
	...
	public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response,
		...
		if (exception instanceof BadCredentialsException) { // 비밀번호 불일치 예외
			loginFailureCount(username);
			errormsg = MessageUtils.getMessage("error.BadCredentials"); 
		} else if (exception instanceof InternalAuthenticationServiceException) { // 유저 정보 존재 예외
			errormsg = MessageUtils.getMessage("error.InternalAuthenticationServiceException");
		} else if (exception instanceof DisabledException) { //
			errormsg = MessageUtils.getMessage("error.Disaled");
		} else if (exception instanceof CredentialsExpiredException) { // 비밀번호 만료 예외
